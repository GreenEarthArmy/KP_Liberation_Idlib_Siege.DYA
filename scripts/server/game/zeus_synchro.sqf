...
while { true } do {
	...
	_zeusunits = [];
	{
		if ((side group _x == GRLIB_side_friendly) && (_x distance startbase > 150) && alive _x) then {
			_zeusunits pushback _x;
		};
	} foreach allUnits;

	{
		if (((typeof _x in _vehicleClassnames) || (_x getVariable ["GRLIB_captured", 0] == 1)) && ((_x distance startbase > 150) && (isNull attachedTo _x) || (typeof _x == huron_typename)) && alive _x ) then {
			_zeusunits pushback _x;
		};
	} foreach vehicles;
